...
static int check_file(int clientfd, struct request_info * file_info)
{
    //
...
static int handle_request(int clientfd, struct request_info * file_info) 
{
	...
	//}
	    ...
	    //
	    code = check_file(clientfd, file_info);
	}
        //
...
int main(int argc, char * argv[])
{
    ...
    static struct request_info info_buffer[1024];
    int nrequests = 0;
    ...
    while (1) {
	   ...
	   struct request_info file_info = info_buffer[nrequests];
	   //void * ptr = Events[i].data.ptr;
	   //uint32_t total_size = Events[i].data.u32;
	   if (Events[i].events == EPOLLIN) {
	       //
	       if (fd == listenfd) {
                   //fd is the listening socket, so accept the connection
                   //and add the client to the interest list
		   ...
		   //info_buffer[i] =
	           ...
	           //
		   request_size = recv(fd, request, BMAX, 0);
                   request[request_size] = '\0';
		   //nrequests += 1;
                   //if (request_size == 0) {
                   //    return;
                   ...
                   int status = handle_request(fd, &file_info);
                   if (status == 0) {
                       //request was fulfilled, so remove the client from the
                       //interest list and close the file descriptor
                       epoll_ctl(epfd, EPOLL_CTL_DEL, fd, NULL);
                       close(fd);
                       //nrequests += 1;
                   }
                   else {
                       //request was a generic GET request, so change
                       //the socket to wait for write readiness
                       //struct epoll_event client_event;
		       
		       file_info.socket = fd;   
		       
 		       info_buffer[nrequests] = file_info;
		       nrequests += 1;

                       memset(&Event, 0x00, sizeof(Event));
                       Event.events = EPOLLOUT;
		       //Event.data.ptr = &file_info;
                       Event.data.fd = fd;
                       //client_event.data.u32 = status;
		       
		       //file_info.fd = ;
		       //file_info.size = status;
	       ...
	       while(1) {
	           //
		   struct request_info info = info_buffer[index];
		   if (info.socket == fd) {
		       //
		       file_info = info;
		       break;
		   ...
		   info_buffer[index] = file_info;

		   epoll_ctl(epfd, EPOLL_CTL_DEL, fd, NULL);
    		   close(fd);		   
	       } else {
	           //
		   amt = read(file_info.fd, buffer, 1024);
		   buffer[amt] = '\0';
		   sent = send(fd, buffer, amt, 0);
		   ssize_t newSize = file_info.size;
		   newSize -= sent;
		   file_info.size = newSize;
